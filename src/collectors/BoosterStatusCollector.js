...
class BoosterStatusCollector {
    ...
    static collectFromHeroUpdate() {
        const originalHeroUpdate = window.shared ? window.shared.Hero.update.bind(window.shared.Hero) : window.Hero.update.bind(window.Hero)
        const hookedUpdate = (field, value, add) => {
            if (field === 'xp') {
                ...
                if (travelMemories) {
                    const {Hero: {infos: {Xp: {cur: oldValue, level}}}} = window.shared ? window.shared : window
